...
export function SeoDirectory({ eyebrow, title, intro, groups }: { eyebrow: string; title: string; intro: string; groups: Group[] }) {
  return <main className="directory-page">
    <header className="site-header area-header">
      ...
      <nav><Link href="/hizmetler">Hizmetler</Link><Link href="/hizmet-bolgeleri">Bölgeler</Link><Link href="/sancaktepe-oto-cekici">Sancaktepe</Link><Link href="/cekmekoy-oto-cekici">Çekmeköy</Link><Link href="/sultanbeyli-oto-cekici">Sultanbeyli</Link><Link href="/pendik-oto-cekici">Pendik</Link><Link href="/iletisim">İletişim</Link></nav>
      <a className="header-call" href={`tel:${siteConfig.phone}`}><Phone size={18} />{siteConfig.phoneDisplay}</a>
      <MobileMenu links={[{ href: "/hizmetler", label: "Hizmetler" }, { href: "/hizmet-bolgeleri", label: "Hizmet Bölgeleri" }, { href: "/sancaktepe-oto-cekici", label: "Sancaktepe Çekici" }, { href: "/cekmekoy-oto-cekici", label: "Çekmeköy Çekici" }, { href: "/sultanbeyli-oto-cekici", label: "Sultanbeyli Çekici" }, { href: "/pendik-oto-cekici", label: "Pendik Çekici" }, { href: "/iletisim", label: "İletişim & Harita" }]} />
    </header>
